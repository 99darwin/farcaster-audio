...
## Security considerations: residual QR phishing risk

The managed-signer SIWN flow (`startManagedSignerFlow` → user approves in Farcaster → `pollSignerStatus` → `completeManagedSignerLogin`) carries a known residual phishing risk that can't be fully closed without changes to the Farcaster client UI. Integrators should understand the trust boundary.

**What the attack looks like.** An attacker calls Juke's public `POST /v1/auth/signer/create` themselves — it's unauthenticated by design, anyone needs to be able to start sign-in — and receives a real, Juke-sponsored Farcaster deeplink. They encode it as a QR and host it on a lookalike surface (`juke-audio.com`, an X post, an in-DM card). A victim scans the QR. The Farcaster client shows an approval screen branded "Juke" because the request is sponsored with Juke's `FARCASTER_APP_FID`. The victim approves. The attacker polls our status endpoint, learns the victim's fid, calls `/v1/auth/login` with the `(fid, signer_uuid)` pair, and receives a Juke session authenticated as the victim.

**Why we can't fully close it server-side.** The Farcaster client doesn't currently surface the *origin* the QR was generated from at approval time — only the registered app name. There's no in-app signal the victim can check to distinguish a QR generated by `juke.audio` from a QR generated by an attacker page. The signed-key-request EIP-712 schema (`requestFid`, `key`, `deadline`) has no slot for a verification code that could be displayed to the user at approval time. Closing the gap completely requires a Farcaster client feature.

**Mitigations we ship.** Partial defenses, listed honestly:

- **Short deadlines** — managed signers expire 10 minutes after registration (was 24h). An attacker has to keep their lookalike up and convert quickly.
- **Per-signer poll cap** — each `signer_uuid` can be polled at most 30 times before it's locked out. Limits asynchronous enumeration of leaked UUIDs.
- **Per-IP rate limit on `/v1/auth/login`** — slows down redemption-side enumeration.
- **Audit logging on `/v1/auth/signer/create`** — every signer-creation event is logged with IP, user-agent, and `signer_uuid`, so abuse patterns are detectable post-hoc.
- **Branding** — the Neynar approval screen shows "Juke" because we sponsor with our `FARCASTER_APP_FID`; a victim scanning a QR for a non-Juke app would see a different name. This deters but doesn't prevent the attack.

**What we explicitly don't do.** We do not display a "verification code" under our QR claiming the user should check it against the Farcaster client. Farcaster doesn't currently show such a code, so any code we displayed would be cargo-cult — it could confuse users and would erode trust the day a real verification mechanism arrives.

**What integrators should do.** Embed Juke from a domain whose origin matches the user's expectation. If you're showing the Juke QR inside your own product, make the trust signal explicit — "Powered by Juke" with attribution back to `juke.audio` — so the user has context for the approval prompt they're about to see.

**Tracked upstream.** A Farcaster-client feature request to surface the requesting origin (or a comparable verification code) at signer-approval time is tracked here: [99darwin/farcaster-audio-client#159](https://github.com/99darwin/farcaster-audio-client/issues/159).
