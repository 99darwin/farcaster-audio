...
class AgentJoinRequest(BaseModel):
    ...
    agent_name: str = Field(
        ..., min_length=1, max_length=128, description="Display name for the agent"
    )
    agent_pfp_url: str | None = Field(
        default=None, max_length=512, description="Optional avatar URL (https only)"
    )
    ...
    def validate_agent_name(cls, v: str) -> str:
        if not _SAFE_NAME_PATTERN.match(v):
            raise ValueError(
                "agent_name must contain only letters, numbers, spaces, hyphens, underscores, and dots"
            )
        return v.strip()
