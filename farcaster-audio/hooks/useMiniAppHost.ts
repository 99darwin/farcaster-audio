...
import {
  ...
  AddMiniApp,
  SignManifest,
} from "@farcaster/miniapp-core/dist/actions";
import type {
  MiniAppHost,
...
export function useMiniAppHost({
  ...
  const sdk: Omit<MiniAppHost, "ethProviderRequestV2"> = useMemo(
    () => ({
      ...
      composeCast: async <C extends boolean | undefined = undefined>(options: {
        text?: string;
      ...
      impactOccurred: async (
        ...
        await Haptics.impactAsync(
          styleMap[type] ?? Haptics.ImpactFeedbackStyle.Medium,
        );
      },
      ...
      notificationOccurred: async (type: "success" | "warning" | "error") => {
        ...
        await Haptics.notificationAsync(
          typeMap[type] ?? Haptics.NotificationFeedbackType.Success,
        );
      },
