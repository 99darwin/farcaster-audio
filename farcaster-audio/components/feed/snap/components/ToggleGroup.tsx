...
export function SnapToggleGroup({ props }: { props: ToggleGroupProps }) {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <View style={[styles.group, { flexDirection: vertical ? 'column' : 'row' }]}>
          ...
          return (
            <Pressable
              ...
              style={[
                styles.option,
                outline
                  ? { borderWidth: 1, borderColor: isSelected ? color : colors.background.border }
                  : null,
                isSelected
                  ? { backgroundColor: outline ? color + '22' : color }
                  : { backgroundColor: outline ? 'transparent' : colors.background.surface },
              ]}
            >
              <Text
                style={[
                  styles.optionLabel,
                  { color: isSelected && !outline ? '#ffffff' : colors.text.body },
                ]}
                numberOfLines={1}
              >
                {option}
